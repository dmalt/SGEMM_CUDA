...
void run_sgemm_naive_dmalt(int M, int N, int K, float alpha, float *A,
...
void run_sgemm_coalesce_dmalt(int M, int N, int K, float alpha, float *A, float *B,
                              float beta, float *C) {
  dim3 gridDim(CEIL_DIV(M, 32), CEIL_DIV(N, 32));
  dim3 blockDim(32 * 32);
  sgemm_coalesce_dmalt<32>
      <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
}
...
void run_sgemm_shared_mem_block_dmalt(int M, int N, int K, float alpha, float *A,
                                float *B, float beta, float *C) {
  dim3 gridDim(CEIL_DIV(M, 32), CEIL_DIV(N, 32));
  dim3 blockDim(32 * 32);
  // L1 cache becomes useless, since we access GMEM only via SMEM, so we carve
...
void runSgemm1DBlocktilingDmalt(int M, int N, int K, float alpha, float *A, float *B,
                           float beta, float *C) {
  const uint BM = 64;
  const uint BN = 64;
  const uint BK = 8;
  ...
  dim3 gridDim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
  dim3 blockDim((BM * BN) / TM);
  sgemm1DBlocktilingDmalt<BM, BN, BK, TM>
      <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
}
...
void runSgemm2DBlocktilingDmalt(int M, int N, int K, float alpha, float *A, float *B,
                           float beta, float *C) {
  const uint BK = 8;
  const uint TM = 8;
  const uint TN = 8;
  if (M >= 128 and N >= 128) {
    ...
    dim3 gridDim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    dim3 blockDim((BM * BN) / (TM * TN));
    sgemm2DBlocktilingDmalt<BM, BN, BK, TM, TN>
        <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
  } else {
    // this is a hacky solution to the underlying problem
    // of not having proper bounds checking in the kernel
...
void runSgemmVectorizeDmalt(int M, int N, int K, float alpha, float *A, float *B,
                       float beta, float *C) {
  const uint BK = 8;
  const uint TM = 8;
  const uint TN = 8;
  if (M >= 128 and N >= 128) {
    ...
    dim3 gridDim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    dim3 blockDim((BM * BN) / (TM * TN));
    sgemmVectorizeDmalt<BM, BN, BK, TM, TN>
        <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
  } else {
    // this is a hacky solution to the underlying problem
    // of not having proper bounds checking in the kernel
...
void runSgemmResolveBankConflictsDmalt(int M, int N, int K, float alpha, float *A,
                                  float *B, float beta, float *C) {
  const uint BK = 8;
  const uint TM = 8;
  const uint TN = 8;
  if (M >= 128 and N >= 128) {
    ...
    dim3 gridDim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    dim3 blockDim((BM * BN) / (TM * TN));
    sgemmResolveBankConflictsDmalt<BM, BN, BK, TM, TN>
        <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
  } else {
    // this is a hacky solution to the underlying problem
    // of not having proper bounds checking in the kernel
...
void run_kernel(int kernel_num, int M, int N, int K, float alpha, float *A,
  ...
  case 21:
    ...
    break;
  case 22:
    run_sgemm_coalesce_indswap_dmalt(M, N, K, alpha, A, B, beta, C);
    break;
  case 23:
  ...
  case 24:
    ...
    break;
  case 25:
    run_sgemm_shared_mem_block_dmalt(M, N, K, alpha, A, B, beta, C);
    break;
  case 26:
    runSgemm1DBlocktilingDmalt(M, N, K, alpha, A, B, beta, C);
    break;
  case 27:
    runSgemm2DBlocktilingDmalt(M, N, K, alpha, A, B, beta, C);
    break;
  case 28:
    runSgemmVectorizeDmalt(M, N, K, alpha, A, B, beta, C);
    break;
  case 29:
    runSgemmResolveBankConflictsDmalt(M, N, K, alpha, A, B, beta, C);
    break;
  default:
