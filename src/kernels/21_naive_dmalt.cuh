...
template <const uint BM, const uint BN, const uint BK, const uint TM, const uint TN>
__global__ void sgemmResolveBankConflictsDmalt(int M, int N, int K, float alpha,
    float *A, float *B, float beta, float *C) {
  __shared__ float As[BM * BK];
  __shared__ float Bs[BK * BN];
  ...
  for (int bkIdx = 0; bkIdx < K; bkIdx += BK) {
    ...
    Bs[innerRowB * BN + ((innerColB * 4 + 0) % TN) * k + (innerColB * 4 + 0) / TN] = tmp.x;
    Bs[innerRowB * BN + ((innerColB * 4 + 1) % TN) * k + (innerColB * 4 + 1) / TN] = tmp.y;
    Bs[innerRowB * BN + ((innerColB * 4 + 2) % TN) * k + (innerColB * 4 + 2) / TN] = tmp.z;
    Bs[innerRowB * BN + ((innerColB * 4 + 3) % TN) * k + (innerColB * 4 + 3) / TN] = tmp.w;
    __syncthreads();
    ...
    for (int iDot = 0; iDot < BK; ++iDot) {
      ...
      for (int iTN = 0; iTN < TN; ++iTN) {
        regTN[iTN] = Bs[iDot * BN + iTN * k + tCol];
      }
